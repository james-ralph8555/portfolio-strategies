...
class TestEquityVolBarbellFunctional:
    ...
    def bull_market_data(self):
        ...
        data["TAIL_Returns"] = np.random.normal(
            -0.0002, 0.008, 252
        )  # Negative drift (cost of hedge)

        # Low VIX environment
        data["VIX"] = (
            15
            + 3 * np.sin(np.linspace(0, 8 * np.pi, 252))
            + np.random.normal(0, 1, 252)
        )
    ...
    def bear_market_data(self):
        ...
        data["TQQQ_Returns"] = np.random.normal(
            -0.003, 0.04, 252
        )  # Negative drift, high vol
    ...
    def volatile_sideways_data(self):
        ...
        data["SVOL_Returns"] = np.random.normal(
            0.0002, 0.008, 252
        )  # Volatility premium
        ...
        data["VIX"] = (
            20
            + 15 * np.sin(np.linspace(0, 6 * np.pi, 252))
            + np.random.normal(0, 3, 252)
        )
        data["VIX_MA20"] = data["VIX"].rolling(window=20).mean()
    ...
    def test_bear_market_workflow(self, strategy, bear_market_data):
        ...
        bear_market_data_with_prices = bear_market_data_with_prices.iloc[
            : len(prices)
        ].copy()
        ...
        drawdown_trigger = strategy.calculate_drawdown_trigger(
            bear_market_data_with_prices
        )
    ...
    def test_rebalancing_workflow(self, strategy):
        ...
        target_weights = {"TQQQ": 0.70, "SVOL": 0.15, "TAIL": 0.10, "SGOV": 0.05}
    ...
    def test_market_regime_changes(self, strategy):
        ...
        all_returns = np.concatenate(
            [regime1_returns, regime2_returns, regime3_returns]
        )
        ...
        vix_values = []
        for i, _ret in enumerate(all_returns):
    ...
    def test_config_updates_workflow(self, strategy):
        ...
        # Original config
        strategy.calculate_weights(pd.DataFrame())
