...
class TestRiskParityStrategyFunctional:
    ...
    def realistic_market_data(self):
        ...
        correlation = -0.3
        tmf_returns = tmf_returns - correlation * tqqq_returns * (
            tmf_daily_vol / tqqq_daily_vol
        )
    ...
    def test_risk_parity_properties(self, strategy, realistic_market_data):
        ...
        risk_contributions = (
            np.multiply(weights_array, marginal_contrib) / portfolio_risk
        )
    ...
    def test_volatility_targeting(self, strategy, realistic_market_data):
        ...
        constrained_metrics = strategy.get_portfolio_metrics(
            realistic_market_data, constrained_weights
        )
    ...
    def test_config_variations(self, realistic_market_data):
        ...
        equity_heavy_weights = equity_heavy_strategy.calculate_weights(
    ...
    def test_error_handling(self, strategy):
        ...
        incomplete_data = pd.DataFrame({"TQQQ": [100, 101, 102]})

        with pytest.raises(ValueError):
            strategy.calculate_weights(incomplete_data)

        # Data with NaN values
        nan_data = pd.DataFrame({"TQQQ": [100, np.nan, 102], "TMF": [100, 101, np.nan]})
