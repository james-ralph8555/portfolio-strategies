...
from tests.fixtures.sample_data import create_sample_config, create_sample_price_data
...
class TestStrategyWorkflow:
    ...
    def test_workflow_with_market_volatility(self):
        ...
        high_vol_data["TQQQ"] = high_vol_prices.reindex(
            high_vol_data.index, method="ffill"
        )
        ...
        low_vol_data["TQQQ"] = low_vol_prices.reindex(
            low_vol_data.index, method="ffill"
        )
    ...
    def test_rebalancing_frequency_impact(self):
        ...
        assert (
            loose_strategy.should_rebalance(drifted_weights, initial_weights) is False
        )
    ...
    def test_volatility_targeting_effectiveness(self):
        ...
        portfolio_returns = pd.Series(
            sum(returns[asset] * weights[asset] for asset in self.strategy.assets)
        )
