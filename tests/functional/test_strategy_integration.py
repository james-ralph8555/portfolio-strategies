...
class TestStrategyIntegration:
    ...
    def test_full_workflow_monthly_rebalancing(self, sample_config, sample_price_data):
        """Test complete workflow with monthly rebalancing."""
        strategy = EquityInflationBetaStrategy(sample_config)
        
        # Initial allocation
        target_weights = strategy.calculate_weights(sample_price_data)
        ...
        if should_rebalance:
            ...
            new_target_weights = strategy.calculate_weights(sample_price_data)
            assert abs(sum(new_target_weights.values()) - 1.0) < 1e-10

    def test_strategy_with_realistic_market_data(self, sample_config):
    ...
    def test_strategy_performance_different_regimes(self, sample_config):
    ...
    def test_rebalancing_frequency_impact(self, sample_config, sample_price_data):
        """Test impact of different rebalancing frequencies."""
        # Test with different drift bands
        tight_bands_config = sample_config.copy()
        tight_bands_config['rebalancing']['drift_bands'] = 5
        
        loose_bands_config = sample_config.copy()
        ...
        target_weights = tight_strategy.calculate_weights(sample_price_data)
    ...
    def test_strategy_with_missing_data_scenarios(self, sample_config):
    ...
    def test_strategy_edge_cases(self, sample_config):
