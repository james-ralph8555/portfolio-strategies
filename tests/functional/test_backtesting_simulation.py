...
class TestBacktestingSimulation:
    ...
    def test_simple_backtest_workflow(self, sample_config):
        ...
        for month in range(1, 13):
            ...
            for _, row in month_returns.iterrows():
                daily_return = sum(
                    current_weights[asset] * row[asset] for asset in current_weights
                )
    ...
    def test_performance_attribution(self, sample_config):
        ...
        strategy.calculate_weights(market_data.iloc[:63])
        ...
        for asset in strategy.assets:
            ...
            asset_return = (
                market_data[asset].iloc[-1] / market_data[asset].iloc[0]
            ) - 1
        ...
        assert all(
            isinstance(contrib, (int, float))
            for contrib in asset_contributions.values()
        )
        ...
        for i in range(60, len(market_data), 5):  # Every 5 days
            ...
            if i + 1 < len(market_data):
                ...
                portfolio_return = sum(
                    weights[asset] * next_day_returns[asset] for asset in weights
                )
    ...
    def test_transaction_costs_impact(self, sample_config):
        ...
        for _, row in returns.iterrows():
            ...
            portfolio_return = sum(
                weights_no_cost[asset] * row[asset] for asset in weights_no_cost
            )
    ...
    def test_scenario_analysis(self, sample_config):
        ...
        scenarios = {
            "crash": {"TQQQ": -0.05, "PDBC": -0.02, "IAU": 0.01, "SGOV": 0.001},
            "recovery": {"TQQQ": 0.04, "PDBC": 0.02, "IAU": 0.005, "SGOV": 0.0005},
            "inflation": {"TQQQ": 0.01, "PDBC": 0.03, "IAU": 0.02, "SGOV": 0.0002},
            "stagnation": {"TQQQ": 0.0005, "PDBC": 0.001, "IAU": 0.0008, "SGOV": 0.001},
        }
        ...
        for scenario_name, daily_returns in scenarios.items():
            ...
            for asset, daily_return in daily_returns.items():
                ...
                asset_returns = np.random.normal(
                    daily_return, abs(daily_return) * 0.5, 30
                )
                prices[asset] = 100 * np.cumprod(1 + asset_returns)
            ...
            scenario_return = sum(
                weights[asset] * daily_returns[asset] for asset in weights
            )
