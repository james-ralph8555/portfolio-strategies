...
class TestEquityInflationBetaStrategy:
    ...
    def test_initialization_with_partial_config(self):
        ...
        partial_config = {"rebalancing": {"frequency": "weekly", "drift_bands": 5}}
    ...
    def test_calculate_trend_signal_insufficient_data(self, strategy):
        ...
        short_data = pd.DataFrame({"PDBC": [100, 101, 102], "IAU": [50, 51, 52]})

        signals = strategy.calculate_trend_signal(short_data)
        assert signals["PDBC"] == 0
        assert signals["IAU"] == 0

    def test_calculate_trend_signal_missing_columns(
    ...
    def test_calculate_carry_signal_missing_columns(
        self, strategy, inflation_beta_price_data
    ):
    ...
    def test_calculate_risk_parity_weights(self, strategy, inflation_beta_price_data):
        ...
        weights = strategy._calculate_risk_parity_weights(
            inflation_beta_price_data, ["PDBC", "IAU"]
        )
    ...
    def test_calculate_risk_parity_weights_missing_columns(
        self, strategy, sample_price_data
    ):
        """Test risk parity weights with missing asset columns."""
        weights = strategy._calculate_risk_parity_weights(
            sample_price_data, ["PDBC", "MISSING"]
        )
    ...
    def test_calculate_portfolio_volatility_missing_tqqq(
        self, strategy, inflation_beta_price_data
    ):
