...
class RiskParityStrategy(Strategy):
    ...
    def __init__(self, config: Optional[Dict] = None):
        ...
        self.rebalance_frequency = self.config.get("rebalancing", {}).get("frequency", "monthly")
        self.drift_bands = self.config.get("rebalancing", {}).get("drift_bands", 5)
        
        # Risk parity parameters
        self.risk_budget = self.config.get("risk_parity", {}).get("risk_budget", {"equity": 0.75, "bond": 0.25})
        self.lookback_period = self.config.get("risk_parity", {}).get("lookback_period", 90)
        self.tolerance = self.config.get("risk_parity", {}).get("optimization", {}).get("tolerance", 1e-10)
        
        # Risk management
        self.max_leverage = self.config.get("risk_management", {}).get("max_leverage", 3.0)
        self.volatility_target = self.config.get("risk_management", {}).get("volatility_target", 0.15)
    ...
    def calculate_weights(self, data: pd.DataFrame) -> Dict[str, float]:
        ...
        returns_data = data[required_assets].iloc[-self.lookback_period:].pct_change().dropna()
        ...
        if len(returns_data) < 10:  # Need minimum data for robust covariance
            ...
            return {asset: equal_weight for asset in required_assets}
        ...
        risk_budget_array = np.array([
            self.risk_budget.get("equity", 0.75),
            self.risk_budget.get("bond", 0.25)
        ])
        ...
        try:
            optimal_weights = self._get_risk_parity_weights(cov_matrix.values, risk_budget_array, initial_weights)
            weights_dict = dict(zip(required_assets, optimal_weights))
        except Exception as e:
            # Fall back to equal weights if optimization fails
            equal_weight = 1.0 / len(required_assets)
            weights_dict = {asset: equal_weight for asset in required_assets}
    ...
    def _get_risk_parity_weights(self, covariances: np.ndarray, 
        ...
        min_weight = self.config.get("risk_parity", {}).get("optimization", {}).get("min_weight", 0.01)
        max_weight = self.config.get("risk_parity", {}).get("optimization", {}).get("max_weight", 0.99)
    ...
    def _apply_risk_constraints(self, weights: Dict[str, float], data: pd.DataFrame) -> Dict[str, float]:
        """
    ...
    def get_portfolio_metrics(self, data: pd.DataFrame, weights: Dict[str, float]) -> Dict[str, float]:
        ...
        risk_contributions = np.multiply(weights_array, marginal_contrib) / portfolio_vol
