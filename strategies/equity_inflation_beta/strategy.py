...
class EquityInflationBetaStrategy(Strategy):
    ...
    def calculate_weights(self, data: pd.DataFrame) -> Dict[str, float]:
        ...
        trend_weight = self.config.get('signals', {}).get('trend', {}).get('weight', 0.6)
        carry_weight = self.config.get('signals', {}).get('carry', {}).get('weight', 0.4)
        tqqq_base_weight = self.config.get('tqqq_overweight', {}).get('base_weight', 0.6)
        scaling_factor = self.config.get('tqqq_overweight', {}).get('scaling_factor', 1.2)
        ...
        for asset in ['PDBC', 'IAU']:
            ...
            diversifier_signals[asset] = (trend_weight * trend_sig + carry_weight * carry_sig)
        ...
        for asset in ['PDBC', 'IAU']:
            ...
            signal_tilt = 0.5 * (1 + diversifier_signals[asset])  # Scale signal to [0, 1]
        ...
        vol_scale = min(scaling_factor, max(0.5, 0.15 / portfolio_vol)) if portfolio_vol > 0 else 1.0
        ...
        if remaining_for_diversifiers > 0 and sum(diversifier_weights.values()) > 0:
            ...
            for asset in diversifier_weights:
                diversifier_weights[asset] = (diversifier_weights[asset] / total_diversifier) * remaining_for_diversifiers
    ...
    def calculate_trend_signal(self, data: pd.DataFrame) -> Dict[str, float]:
        ...
        """
        lookback_periods = self.config.get('signals', {}).get('trend', {}).get('lookback_periods', [20, 60, 120])
        signals = {}
            ...
            if asset in data.columns:
                ...
                for period in lookback_periods:
                    if len(data) >= period:
                        ...
                        signal = np.sign(returns.iloc[-1]) if not pd.isna(returns.iloc[-1]) else 0
    ...
    def validate_config(self) -> bool:
        ...
        """
        required_keys = ['assets', 'signals', 'risk_parity', 'tqqq_overweight', 'volatility_targeting', 'rebalancing']
    ...
    def _calculate_risk_parity_weights(self, data: pd.DataFrame, assets: List[str]) -> Dict[str, float]:
        """
        ...
        for asset in assets:
            if asset in data.columns:
                ...
                if len(returns) >= lookback_period:
                    ...
                    volatilities[asset] = vol if vol > 0 else 0.15  # Default vol if zero
                else:
    ...
    def _calculate_portfolio_volatility(self, data: pd.DataFrame) -> float:
        ...
        """
        lookback_period = self.config.get('volatility_targeting', {}).get('lookback_period', 60)
