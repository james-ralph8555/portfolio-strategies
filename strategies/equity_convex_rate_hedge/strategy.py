...
class EquityConvexRateHedgeStrategy(Strategy):
    ...
    def _calculate_stock_bond_correlation(self, data: pd.DataFrame) -> float:
        ...
        if "TQQQ" in data.columns and "PFIX" in data.columns:
            ...
            if len(stock_returns) >= lookback and len(bond_returns) >= lookback:
                correlation = stock_returns.tail(lookback).corr(bond_returns.tail(lookback))
    ...
    def _apply_volatility_targeting(self, weights: Dict[str, float], 
                                   vol_scale: float) -> Dict[str, float]:
        """
    ...
    def validate_config(self) -> bool:
        ...
        """
        required_keys = [
            "target_volatility", "tqqq_base_weight", "pfix_base_weight",
            "gold_base_weight", "cash_base_weight", "correlation_threshold",
            "volatility_lookback", "correlation_lookback", "drift_bands"
        ]
