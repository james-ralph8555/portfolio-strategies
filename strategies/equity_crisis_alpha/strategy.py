...
class EquityCrisisAlphaStrategy(Strategy):
    ...
    def __init__(self, config: Optional[Dict] = None):
        ...
        self.drift_bands = self.config.get('rebalancing', {}).get('drift_bands', 10)  # 10-point drift bands
    ...
    def calculate_weights(self, data: pd.DataFrame) -> Dict[str, float]:
        ...
        risk_budget = self.config.get('risk_budget', {
            'tqqq_weight': 0.6,
    ...
    def _calculate_erc_weights(self, cov_matrix: pd.DataFrame, risk_budget: Dict) -> Dict[str, float]:
        """
        ...
        for i, asset in enumerate(self.assets):
            if asset == "TQQQ":
                ...
                weights[i] = risk_budget['diversifier_weight'] / 2  # Split between diversifiers
            elif asset == "SGOV":
                weights[i] = risk_budget['cash_weight']
        
        return dict(zip(self.assets, weights))
    
    def _apply_black_litterman_tilt(self, weights: Dict[str, float], returns: pd.DataFrame) -> Dict[str, float]:
        """
        ...
        for view in views:
            if 'TQQQ' in view.get('assets', []):
                ...
                if total_tilt != 0:
                    other_assets = [asset for asset in tilted_weights if asset != 'TQQQ']
                    other_total = sum(weights[asset] for asset in other_assets)
                    
                    for asset in other_assets:
                        if other_total > 0:
                            weight_ratio = weights[asset] / other_total
                            tilted_weights[asset] = weights[asset] - (total_tilt * weight_ratio)
        
        return tilted_weights
    
    def _apply_volatility_targeting(self, weights: Dict[str, float], returns: pd.DataFrame) -> Dict[str, float]:
        """
        ...
        # Calculate portfolio volatility
        portfolio_vol = np.sqrt(np.dot(weight_array.T, np.dot(cov_matrix.loc[self.assets, self.assets], weight_array)))
        ...
        return dict(zip(self.assets, scaled_weights))
