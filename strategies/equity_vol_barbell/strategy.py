...
class EquityVolBarbellStrategy(Strategy):
    ...
    def calculate_weights(self, data: pd.DataFrame) -> Dict[str, float]:
        ...
        cash = barbell_config.get("cash_weight", 0.05)
        ...
        svol_weight = min(vol_sleeves.get("SVOL", short_vol) or short_vol, 
                         self.config.get("vol_sizing", {}).get("svol_max_weight", 0.2))
        tail_weight = min(vol_sleeves.get("TAIL", tail_hedge) or tail_hedge, 
    ...
    def calculate_drawdown_trigger(self, data: pd.DataFrame) -> float:
        ...
        if current_drawdown < -max_drawdown_threshold or vol_ratio > vol_spike_threshold:
    ...
    def size_vol_sleeves(self, data: pd.DataFrame) -> Dict[str, float]:
        ...
        svol_base = self.config.get("barbell_allocation", {}).get("short_vol_weight", 0.15)
        tail_base = self.config.get("barbell_allocation", {}).get("tail_hedge_weight", 0.1)
        
        # Adjust SVOL based on VIX term structure (if VIX data available)
        if "VIX" in data.columns:
            vix_current = data["VIX"].iloc[-1]
            vix_ma = data["VIX"].rolling(50).mean().iloc[-1] if len(data) > 50 else data["VIX"].mean()
        ...
        if "VIX" in data.columns:
            vix_percentile = data["VIX"].rolling(252).rank(pct=True).iloc[-1] if len(data) > 252 else 0.5
    ...
    def validate_config(self) -> bool:
        ...
        required_sections = ["barbell_allocation", "drawdown_triggers", "vol_sizing", "rebalancing"]
        ...
        total = (barbell.get("tqqq_base_weight", 0) + 
        ...
        for key, value in barbell.items():
            if not (0 <= value <= 1):
                return False
