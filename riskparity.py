...
def assets_risk_contribution_to_allocation_risk(weights, covariances): #returns contribution of each asset to risk of weight distribution
    portfolio_risk = allocation_risk(weights, covariances)
    return np.multiply(weights.T, covariances * weights.T) / portfolio_risk

def risk_budget_objective_error(weights, args): #returns error between desired contribution and calculated contribution of each asset
    covariances = args[0]
...
def get_risk_parity_weights(covariances, assets_risk_budget, initial_weights):
    ...
    return minimize(fun=risk_budget_objective_error,
                    x0=initial_weights,
...
def get_weights_yahoo(yahoo_tickers=['TQQQ', 'TMF'],
    ...
    for i in range(len(yahoo_tickers)):
        data = yf.download(yahoo_tickers[i], start=start_date, end=end_date, interval='1d')
    ...
    weights = pd.Series(get_risk_parity_weights(covariances, assets_risk_budget, init_weights), index=prices.columns, name='weight')
...
if __name__ == '__main__':
    ...
    try:
        ...
        raise Exception("Inputs must be floats")
    assert len(current) == 2
    pv = sum(current)
    sdata = get_weights_yahoo(yahoo_tickers=['QQQ', 'TLT'], assets_risk_budget = [0.75, 0.25])
    pdata = get_weights_yahoo(yahoo_tickers=['TQQQ', 'TMF'], assets_risk_budget = [0.75, 0.25])
    ...
    print(f"TQQQ Allocation: {round(weights[0] * 100, 2)}%, TMF Allocation: {round(weights[1] * 100, 2)}%")
    ...
    print(f'TQQQ D: ${(pv*weights[0])-current[0]} TMF D: {(pv*weights[1])-current[1]}')
...
plt.plot(np.linspace(1986, 2019, len(portfolio[1])), portfolio[1], color='r', linewidth=0.33)
plt.yscale('log')
...
plt.plot(np.linspace(1986, 2019, len(portfolio[1])), portfolio[1], color='b', linewidth=0.33)
plt.yscale('log')
...
plt.plot(np.linspace(1986, 2019, len(portfolio[1])), portfolio[1], color='g', linewidth=0.33)
plt.yscale('log')
#portfolio = backtest_csv(prices = prices, td = 7, assets_risk_budget = [0.65, 0.35])
#print(portfolio[0])
#plt.figure(5, figsize=(8, 6), dpi=80, facecolor='w', edgecolor='k')
#plt.plot(np.linspace(1986, 2019, len(portfolio[1])), portfolio[1], color='c', linewidth=0.25)
#plt.yscale('log')
"""
